...
// settings
const int max_intensity = 255;    // max of 255
const int min_intensity = 5;     
const int duration = 30000;       // time it takes to go back and forth between max and min intensity
const int inbetween_time = 25;   // off time in between the 3 individual white leds in a smd5050
...
void setup()
{
    ...
    white_1_ramp.go(max_intensity, duration, SINUSOIDAL_INOUT, LOOPFORWARD);    // go to value 30 in 1000ms in a linear line and looping up and down

    white_2_ramp.setGrain(1);                         // set grain to 1 ms
    white_2_ramp.go(min_intensity);                               // make sure to start at 0
    white_2_ramp.go(max_intensity, duration, SINUSOIDAL_INOUT, LOOPFORWARD);    // go to value 30 in 1000ms in a linear line and looping up and down

    white_3_ramp.setGrain(1);                         // set grain to 1 ms
    white_3_ramp.go(min_intensity);                               // make sure to start at 0
    white_3_ramp.go(max_intensity, duration, SINUSOIDAL_INOUT, LOOPFORWARD);    // go to value 30 in 1000ms in a linear line and looping up and down       
